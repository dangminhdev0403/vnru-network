...
| **OPEN-03** | **Knowledge & Expert Moderation Governance**<br>Who has authority to publish, edit, or curate Knowledge Repository records and Expert profiles? Is there an institutional moderation queue, automated external sync (ORCID/Scopus), or direct self-publication? | Knowledge / Expert | Affects the Knowledge Publications and Directory module state machines (draft → moderation → published), approval workflows, and role permissions. | **Dev baseline: 2026-08-20** — Module 2 MVP is read-only public discovery. No create/edit/publish/moderation API; synthetic fixtures explicitly marked public are the only exposed records. Production governance remains unresolved. |
| **OPEN-04** | **Semantic Search & Partner Matching Engine**<br>Which search and matching technologies should power semantic search and partner recommendations (e.g., PostgreSQL pgvector, Elasticsearch/OpenSearch, dedicated Graph DB, or hybrid)? | Search / Data Infra | Affects data indexing pipelines, asynchronous event consumers, infrastructure footprint, query performance, and recommendation accuracy. | **Dev baseline: 2026-08-20** — Use PostgreSQL-native text search and deterministic shared-topic/expertise matching. Add a dedicated engine only when representative p95 exceeds 300 ms or agreed relevance fixtures fail. Production architecture remains unresolved. |
| **OPEN-05** | **Paired Proposal Collaboration & Joint Confirmation**<br>For VN–RU joint research proposals, what is the exact mechanism for Vietnamese and Russian teams to jointly draft, review, and mutually confirm/lock the proposal before submission? | Research Collab / UX | Affects `collaboration-service` concurrency control, optimistic locking vs collaborative editing, draft versioning, and submission state machine. | Unresolved |
| **OPEN-06** | **Peer Review Anonymization & Disclosure Rules**<br>Which specific fields must be masked during independent/anonymized peer review, and what are the rules/authority for revealing information post-evaluation if required by policy? | Review / Security | Affects the Collaboration Reviews module data projection, authorization boundaries, proposal snapshotting, audit logging, and conflict-of-interest checks. | Unresolved |
| **OPEN-07** | **Project Milestone & Progress Acceptance Hierarchy**<br>Who has authority to submit, review, and approve research project milestones and progress reports (e.g., Lead Researcher, Organization Representative, Foundation Program Manager)? | Project / PMS | Affects the Collaboration Projects module workflow states, approval chains, and role-based milestone sign-offs. | Unresolved |
...
> **Scope Boundary Decision `[DECISION]`**: All monetary and financial workflows (funding instruments, investment management, budgets, disbursement, payment processing, accounting, sponsor administration, financial reporting, deal values, royalties/licensing fees) are explicitly **excluded from implementation scope** by project-owner decision. They are not treated as open questions.
