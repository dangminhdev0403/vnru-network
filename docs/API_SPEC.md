...
| **3. Grants, Review & PMS** | `/api/v1/grants/opportunities/*`<br>`/api/v1/grants/proposals/*`<br>`/api/v1/reviews/assignments/*`<br>`/api/v1/reviews/evaluations/*`<br>`/api/v1/projects/*` | Publish independent funding opportunity, draft paired joint proposal, confirm VN–RU participation, submit review score, update project milestone. |
...
| `grant` | `grants.opportunity.published` | Bilateral funding opportunity opened | `notification-service`, `analytics-service` |
| `grant` | `grants.proposal.submitted` | Joint proposal confirmed by both sides and submitted | `review-service`, `analytics-service` |
| `review` | `reviews.evaluation.submitted` | Reviewer completed evaluation score & comments | `grant-service`, `analytics-service` |
| `grant` | `grants.decision.approved` | Foundation funding decision finalized | `project-service`, `notification-service`, `analytics-service` |
...
- Grant & transfer decisions: `grants.decision.recorded`, `technology.consortium.slot_confirmed`, `analytics.kpi_definition.updated`, `analytics.report.exported`.
