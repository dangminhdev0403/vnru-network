...
Auth.js authenticates a person from the runtime account config. `auth-service` resolves the signed identity assertion to one opaque application session, one active authorization context, roles, resource scopes and capability keys.
