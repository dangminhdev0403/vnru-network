...
- The typed workspace registry owns member, account, security, and governance navigation.
- Business roles do not create separate frontend applications or workflow pages. All non-administrator members land on `/workspace`.
...
- Authenticated users holding multiple roles keep the active authorization context; they do not switch dashboard personas.
- Context changes update the active session authorization token and refresh capability-gated navigation without creating workflow UI.
