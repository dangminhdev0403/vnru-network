...
import { projectServiceUrl, backendHeaders } from "@/features/auth/server";

async function proxy(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  try {
    const { path } = await params;
    const url = new URL(request.url);
    const targetUrl = new URL(projectServiceUrl(`api/v1/projects/${path.map(encodeURIComponent).join("/")}`));
