...
import { collabServiceUrl, backendHeaders } from "@/features/auth/server";

async function proxy(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  try {
    const { path } = await params;
    const url = new URL(request.url);
    const targetUrl = new URL(collabServiceUrl(`api/v1/collab/${path.map(encodeURIComponent).join("/")}`));
