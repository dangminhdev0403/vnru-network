...
test("registration form posts to the same-origin membership API with feedback", async () => {
  const [page, route] = await Promise.all([
    read("app/register/page.tsx"),
    read("app/api/membership-applications/route.ts"),
  ]);

  assert.match(page, /fetch\("\/api\/membership-applications"/);
  ...
  assert.match(route, /api\/v1\/membership-applications/);
});
