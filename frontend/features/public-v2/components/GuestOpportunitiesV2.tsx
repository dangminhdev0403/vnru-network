...
const COPY: Record<Locale, {
  ...
  vi: {
    ...
    title3: "Nga – Việt",
    intro: "Khám phá các chương trình kêu gọi đề xuất đề tài song phương, tài trợ nghiên cứu học thuật độc lập và thành lập nhóm nghiên cứu chung giữa các viện và trường đại học Việt Nam – Liên bang Nga.",
    searchPlaceholder: "Tìm kiếm đề tài theo tên chương trình, mã đề xuất, từ khóa, chuyên ngành...",
    searchBtn: "Tìm kiếm",
    ...
    howItWorksDesc: "Quy trình 4 bước tinh gọn giúp các nhà khoa học và tổ chức tiếp cận đề tài nghiên cứu khoa học song phương.",
    step1Title: "1. Khám phá & Chọn đề tài",
    step1Desc: "Tra cứu các chương trình kêu gọi đề tài phù hợp với năng lực và định hướng nghiên cứu của bạn.",
    step2Title: "2. Ghép nối chuyên gia đối tác",
    step2Desc: "Kết nối với đồng tác giả hoặc phòng thí nghiệm tại Việt Nam hoặc Liên bang Nga qua mạng lưới chuyên gia.",
    step3Title: "3. Thuyết minh & Thẩm định",
    step3Desc: "Soạn thảo đề xuất khoa học chung và gửi thẩm định qua hội đồng chuyên môn độc lập của Mạng lưới.",
    step4Title: "4. Triển khai & Công bố",
    step4Desc: "Tiến hành nghiên cứu, chia sẻ dữ liệu thử nghiệm và công bố kết quả khoa học trên các tạp chí uy tín.",
    ctaTitle: "Tổ chức của bạn muốn công bố đề tài hoặc mời hợp tác mới?",
    ctaDesc: "Đăng nhập với vai trò Đại diện Tổ chức hoặc Điều phối viên để tạo chương trình hợp tác mới và tiếp cận các nhà khoa học hàng đầu.",
  ...
  en: {
    ...
    intro: "Explore open calls for bilateral proposals, independent academic initiatives, and joint research-group formations across leading institutes and universities in Vietnam and the Russian Federation.",
    searchPlaceholder: "Search opportunities by title, code, keywords, discipline...",
    searchBtn: "Search",
    ...
    howItWorksDesc: "A streamlined 4-step framework to connect scientists, laboratories, and bilateral research programmes.",
    step1Title: "1. Discover & Select Call",
    step1Desc: "Browse open research opportunities aligned with your academic focus and equipment capacity.",
    step2Title: "2. Pair with Bilateral Partners",
    step2Desc: "Connect with co-investigators or partner institutes in Vietnam or Russia via our verified expert directory.",
    step3Title: "3. Submit & Independent Peer Review",
    step3Desc: "Draft your joint proposal and undergo evaluation by the independent peer-review council.",
    step4Title: "4. Execute & Joint Publication",
    step4Desc: "Conduct research, exchange observational datasets, and co-author high-impact publications.",
    ctaTitle: "Looking to launch a new research call or joint laboratory?",
    ctaDesc: "Sign in with an Organization Representative or Coordinator persona to publish bilateral calls and recruit top scientific talent.",
  ...
  ru: {
    ...
    howItWorksDesc: "Пошаговый процесс взаимодействия научных групп и исследовательских организаций двух стран.",
    step1Title: "1. Выбор направления",
    step1Desc: "Изучение открытых конкурсов, соответствующих научному профилю вашей лаборатории.",
    step2Title: "2. Подбор партнеров",
    step2Desc: "Поиск соавторов и партнеров в России или Вьетнаме через верифицированную базу экспертов.",
    step3Title: "3. Подготовка и экспертиза",
    step3Desc: "Формирование совместной заявки и прохождение независимой научной экспертизы.",
    step4Title: "4. Проведение исследований",
    step4Desc: "Выполнение работ, обмен научными данными и совместные публикации в рецензируемых журналах.",
    ctaTitle: "Хотите объявить новый конкурс или создать совместную лабораторию?",
    ctaDesc: "Войдите как представитель организации или координатор для публикации программ и привлечения исследователей.",
    ...
    return DEMO_OPPORTUNITIES.filter((opp) => {
      ...
      if (selectedField !== "all") {
        ...
        if (target?.matchKeyword) {
          const content = `${opp.field} ${opp.title} ${opp.topics?.join(" ") || ""}`.toLowerCase();
          if (!content.includes(target.matchKeyword)) return false;
        }
      }
      // Query filter
      if (q) {
        const text = `${opp.title} ${opp.code} ${opp.field} ${opp.summary || ""} ${opp.topics?.join(" ") || ""}`.toLowerCase();
          ...
          <div className="relative mx-auto max-w-[1460px]">
            <div className="max-w-4xl">
              ...
              <div className="mt-7 flex flex-wrap items-center gap-x-6 gap-y-3 text-xs sm:text-sm md:text-[15px] font-bold text-slate-700">
                <span className="inline-flex items-center gap-2"><b className="text-base text-blue-600">◉</b> 300+ Đề tài đã kết nối</span>
                <span className="inline-flex items-center gap-2"><b className="text-base text-blue-600">◎</b> 14 Lĩnh vực ưu tiên 2026</span>
                <span className="inline-flex items-center gap-2"><b className="text-base text-blue-600">◇</b> 100% Học thuật & Nghiên cứu</span>
                <span className="inline-flex items-center gap-2"><b className="text-base text-blue-600">✦</b> Chương trình VAST – RAS</span>
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="rounded-2xl border border-blue-200/80 bg-white/90 p-5 shadow-[0_16px_40px_-32px_rgba(37,99,235,.4)]">
              ...
              <div className="mt-4 flex flex-col items-start justify-between gap-4 border-t border-blue-100 pt-4 sm:flex-row sm:items-center">
                ...
                <div className="text-xs sm:text-sm font-extrabold text-slate-600">
                  <span className="text-blue-700">{filteredOpportunities.length}</span> {t.resultsCount}
                </div>
              </div>
            </div>
                    ...
                    <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                      <div className="max-w-4xl min-w-0 flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2.5">
                          <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-black ${
                            isOpen ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-amber-50 text-amber-700 border border-amber-200"
                          }`}>
                            <span className={`size-2 rounded-full ${isOpen ? "bg-emerald-500" : "bg-amber-500"}`} />
                        ...
                        {opp.eligiblePartners && (
                          <div className="mt-4 rounded-xl bg-blue-50/70 p-3.5 text-xs sm:text-sm text-slate-700">
                            <strong className="font-bold text-slate-900">{t.eligibleLabel}</strong> {opp.eligiblePartners}
                          </div>
                        )}
                        ...
                        {opp.topics && (
                          <div className="mt-4 flex flex-wrap gap-2">
                            {opp.topics.map((topic) => (
                              <span key={topic} className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-700">
                                #{topic}
                              </span>
                            ))}
                          </div>
                      ...
                      <div className="flex shrink-0 flex-col items-start gap-4 border-t border-blue-100 pt-5 lg:w-72 lg:items-end lg:border-t-0 lg:pt-0">
                        <div className="text-xs sm:text-sm font-semibold text-slate-600 lg:text-right">
                          <span className="block text-xs font-bold uppercase tracking-wider text-slate-400">{t.deadlineLabel}</span>
                          <strong className="mt-1 block text-base font-black text-slate-900">{opp.closes}</strong>
                        </div>
            ...
            <div className="mt-16 rounded-3xl border border-blue-200/90 bg-gradient-to-b from-[#e0efff] to-[#eff6ff] p-8 sm:p-12 shadow-[0_20px_50px_-32px_rgba(37,99,235,.4)]">
              ...
              <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  <div key={step.title} className="rounded-2xl border border-blue-200/80 bg-white/95 p-6 shadow-xs">
