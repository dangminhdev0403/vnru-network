...
const EXPERTS_LIST: ExpertItem[] = [...EXPERTS_RAW].sort((a, b) => a.name.localeCompare(b.name, "vi"));
...
const ORGANIZATIONS_LIST: OrganizationItem[] = [...ORGANIZATIONS_RAW].sort((a, b) => a.name.localeCompare(b.name, "vi"));
...
const KHAISANG_RESULTS = [
  {
    ...
    topic: "Thuật toán định vị và dẫn đường cho hệ thống thiết bị bay tự hành (UAV)",
  ...
  },
  {
    ...
    topic: "Nghiên cứu vật liệu nano oxit kim loại ứng dụng trong lưu trữ năng lượng xanh",
  ...
  },
  {
    ...
    topic: "Mô hình hóa động lực học biển và biến đổi môi trường sinh thái vịnh Bắc Bộ",
  ...
  },
  {
    ...
    topic: "Ứng dụng chùm tia ion năng lượng cao trong phân tích cấu trúc vi mô",
...
export function GuestEcosystemV2({
  ...
  const [activeSection, setActiveSection] = useState<EcosystemTabId>("opportunities");
  const isScrollingRef = useRef(false);
  const scrollTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Sub-tabs for Opportunities
  const [oppSubTab, setOppSubTab] = useState<"connect" | "call-for-papers" | "grants">("connect");
  // Sub-tabs for Members
  const [memberSubTab, setMemberSubTab] = useState<"experts" | "organizations">("experts");
  // Sub-tabs for Library
  const [libSubTab, setLibSubTab] = useState<"articles" | "journals" | "patents">("articles");
  ...
  const [connectErrors, setConnectErrors] = useState<Partial<Record<ConnectField, string>>>({});
  ...
  return (
    <div className="min-h-screen bg-[#edf3f9] text-slate-900 font-sans selection:bg-blue-600 selection:text-white">
      ...
      {/* ═══════════ STICKY SUB-TABS BAR ═══════════ */}
      <nav
        className="sticky top-[74px] z-40 border-b border-blue-200/70 bg-[#edf3f9]/90 py-3.5 backdrop-blur-md transition-all duration-200"
      ...
      <main id="main-content">
        ...
        <section
          id="opportunities"
          className="scroll-mt-32 px-4 pt-10 pb-16 sm:px-6 sm:pt-12 sm:pb-20 lg:px-8 lg:pt-14 lg:pb-24"
        >
          <div className="mx-auto max-w-[1460px]">
            ...
            <div className="text-center mb-10">
              ...
              <p className="mt-3 text-base text-slate-600 sm:text-lg max-w-2xl mx-auto">
                Cầu nối tương tác trực tiếp giúp các nhà khoa học, viện nghiên cứu và trường đại học kết nối, nhận tài trợ và công bố đề tài.
              </p>
            </div>

            {/* Inner Sub-tab Switcher: Kết nối | Công bố | Tài trợ */}
            <div className="flex justify-center mb-10">
              <div className="inline-flex rounded-2xl border border-blue-200 bg-white p-1.5 shadow-xs">
                <button
                  type="button"
                  onClick={() => setOppSubTab("connect")}
                  className={`rounded-xl px-5 py-2.5 text-sm font-black transition-all sm:px-6 sm:text-base ${
                    oppSubTab === "connect"
                ...
                {connectSubmitted ? (
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center">
                    ...
                    <p className="mt-2 text-sm text-emerald-800 sm:text-base">
                      Cảm ơn <strong>{connectForm.fullName}</strong>. Chúng tôi đã tiếp nhận thông tin và sẽ gửi phản hồi đến email <strong>{connectForm.email}</strong> sớm nhất.
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  <form noValidate onSubmit={handleConnectSubmit} className="space-y-6">
                    <div className="grid gap-6 sm:grid-cols-2">
                      <div>
                        <label htmlFor="connect-fullname" className="block text-sm font-bold text-slate-800">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            setConnectForm({ ...connectForm, fullName: e.target.value });
                            setConnectErrors({ ...connectErrors, fullName: undefined });
                          }}
                          aria-invalid={!!connectErrors.fullName}
                          aria-describedby={connectErrors.fullName ? "connect-fullname-error" : undefined}
                        ...
                        />
                        {connectErrors.fullName && <p id="connect-fullname-error" className="mt-1 text-sm font-semibold text-red-600">{connectErrors.fullName}</p>}
                      </div>
                      <div>
                        <label htmlFor="connect-org" className="block text-sm font-bold text-slate-800">
                          Cơ quan / Viện nghiên cứu / Trường ĐH <span className="text-red-500">*</span>
                        </label>
                        <input
                          id="connect-org"
                          type="text"
                          ...
                          onChange={(e) => {
                            setConnectForm({ ...connectForm, organization: e.target.value });
                            setConnectErrors({ ...connectErrors, organization: undefined });
                          }}
                          aria-invalid={!!connectErrors.organization}
                          aria-describedby={connectErrors.organization ? "connect-org-error" : undefined}
                          placeholder="Tên viện / trường / doanh nghiệp..."
                          className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50/50 px-4 py-3 text-base text-slate-900 outline-none! focus-visible:border-blue-600"
                        />
                        {connectErrors.organization && <p id="connect-org-error" className="mt-1 text-sm font-semibold text-red-600">{connectErrors.organization}</p>}
                      </div>
                    </div>

                    <div className="grid gap-6 sm:grid-cols-2">
                      <div>
                        <label htmlFor="connect-email" className="block text-sm font-bold text-slate-800">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setConnectErrors({ ...connectErrors, email: undefined });
                          }}
                          aria-invalid={!!connectErrors.email}
                          aria-describedby={connectErrors.email ? "connect-email-error" : undefined}
                          placeholder="email@institution.edu.vn"
                          className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50/50 px-4 py-3 text-base text-slate-900 outline-none! focus-visible:border-blue-600"
                        />
                        {connectErrors.email && <p id="connect-email-error" className="mt-1 text-sm font-semibold text-red-600">{connectErrors.email}</p>}
                      </div>
                      <div>
                        <label htmlFor="connect-field" className="block text-sm font-bold text-slate-800">
                        ...
                        <select
                          ...
                          className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50/50 px-4 py-3 text-base text-slate-900 outline-none! focus-visible:border-blue-600"
                        >
                          <option value="Vật liệu tiên tiến & Năng lượng">Vật liệu tiên tiến & Năng lượng mới</option>
                          <option value="Trí tuệ nhân tạo & Dữ liệu lớn">Trí tuệ nhân tạo & Dữ liệu lớn (AI/Data)</option>
                          <option value="Hàng không vũ trụ & Điều khiển tự động">Hàng không vũ trụ & Điều khiển tự động (UAV)</option>
                          <option value="Công nghệ Lượng tử">Công nghệ Lượng tử & Cảm biến</option>
                          <option value="Khoa học Biển & Môi trường">Khoa học Biển & Biến đổi khí hậu</option>
                          <option value="Công nghệ Hạt nhân & Y sinh">Công nghệ Hạt nhân & Y sinh</option>
                          <option value="Khoa học Xã hội & Dịch thuật Chuyên khảo">Khoa học Xã hội, Ngôn ngữ & Dịch thuật</option>
                        </select>
                      </div>
                    </div>

                    <div>
                      <label htmlFor="connect-message" className="block text-sm font-bold text-slate-800">
                        Nội dung đề xuất hợp tác / Nhu cầu kết nối <span className="text-red-500">*</span>
                      </label>
                      <textarea
                        id="connect-message"
                        rows={4}
                        ...
                        onChange={(e) => {
                          ...
                          setConnectErrors({ ...connectErrors, message: undefined });
                        }}
                        aria-invalid={!!connectErrors.message}
                        aria-describedby={connectErrors.message ? "connect-message-error" : undefined}
                      ...
                      />
                      {connectErrors.message && <p id="connect-message-error" className="mt-1 text-sm font-semibold text-red-600">{connectErrors.message}</p>}
                    </div>
            ...
            {oppSubTab === "call-for-papers" && (
              <div className="grid gap-6 md:grid-cols-3">
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Chương trình Học bổng Nghiên cứu InteRussia Fellowships 2026
                    </h3>
                    <p className="mt-3 text-sm leading-relaxed text-slate-600">
                      Chương trình học bổng uy tín dành cho các nhà khoa học, chuyên gia trẻ quốc tế sang nghiên cứu trực tiếp tại các viện và trường ĐH hàng đầu của Nga. Tài trợ toàn phần vé máy bay, chỗ ở tại Moskva và sinh hoạt phí.
                    </p>
                  </div>
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Hội nghị Khoa học Quốc tế Song phương VAST – RAS (Nga – Việt)
                    </h3>
                    <p className="mt-3 text-sm leading-relaxed text-slate-600">
                      Kêu gọi gửi bài báo tham luận khoa học cho Hội thảo khoa học quốc tế thường niên giữa Viện Hàn lâm KHCN Việt Nam và Viện Hàn lâm Khoa học Nga trong các lĩnh vực vật liệu, công nghệ sinh học và chuyển đổi số.
                    </p>
                  </div>
                  <div className="mt-6 border-t border-slate-100 pt-4">
                    <Link
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Chuyên san Hợp tác Khoa học Song phương Việt Nam – Liên bang Nga
                    </h3>
                    <p className="mt-3 text-sm leading-relaxed text-slate-600">
                      Đợt tiếp nhận công trình nghiên cứu gốc từ các nhóm tác giả liên kết hai nước đăng trên chuyên san đặc biệt của các tạp chí khoa học quốc tế uy tín thuộc hệ thống RAS & VAST.
                    </p>
                  </div>
                  <div className="mt-6 border-t border-slate-100 pt-4">
                    <Link
            ...
            {oppSubTab === "grants" && (
              <div className="grid gap-6 md:grid-cols-3">
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Quỹ Hỗ trợ Ngoại giao Công chúng Alexander Gorchakov (LB Nga)
                    </h3>
                    <p className="mt-3 text-sm leading-relaxed text-slate-600">
                      Cấp kinh phí tài trợ cho các đề tài nghiên cứu quốc tế, hội thảo khoa học và dự án giao lưu học thuật giữa các chuyên gia, tổ chức phi chính phủ và viện nghiên cứu Việt Nam – LB Nga.
                    </p>
                  </div>
                  <div className="mt-6 border-t border-slate-100 pt-4">
                    <a
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Gói Tài trợ Đề tài Nghiên cứu Cơ bản Song phương Nga – ASEAN
                    </h3>
                    <p className="mt-3 text-sm leading-relaxed text-slate-600">
                      Thông báo từ Bộ Khoa học và Giáo dục Đại học Liên bang Nga (Minobrnauki) tài trợ các nhóm nghiên cứu phối hợp giữa các trường đại học Nga và các nhà khoa học Việt Nam trong lĩnh vực công nghệ xanh và AI.
                    </p>
                  </div>
                  <div className="mt-6 border-t border-slate-100 pt-4">
                    <a
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Tài trợ Ươm tạo Sáng chế & Chuyển giao Công nghệ Nga – Việt
                    </h3>
                    <p className="mt-3 text-sm leading-relaxed text-slate-600">
                      Gói hỗ trợ tài chính và pháp lý giúp thương mại hóa các phát minh, đăng ký bằng độc quyền sáng chế song phương và ứng dụng thực tiễn vào doanh nghiệp hai nước.
                    </p>
                  </div>
                  <div className="mt-6 border-t border-slate-100 pt-4">
                    <Link
        ...
        <section
          id="members"
          className="scroll-mt-32 border-t border-blue-200/50 bg-[#e7f0fa] px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24"
        >
          <div id="network-directory" className="mx-auto max-w-[1460px]">
            <div className="text-center mb-10">
              ...
              <p className="mt-3 text-base text-slate-600 sm:text-lg max-w-2xl mx-auto">
                Danh bạ các nhà khoa học, chuyên gia và các viện hàn lâm, trường đại học đối tác Nga – Việt.
              </p>
            </div>
        ...
        >
          <div className="mx-auto max-w-[1460px]">
            <div className="text-center mb-10">
              ...
              <p className="mt-3 text-base text-slate-600 sm:text-lg max-w-2xl mx-auto">
                Các chương trình học bổng, đề tài trọng điểm của Quỹ và các dự án nghiên cứu song phương đang triển khai.
              </p>
            </div>
            ...
            <div className="space-y-4 max-w-5xl mx-auto">
              ...
              <article className="flex flex-col gap-5 rounded-2xl border border-blue-200/90 bg-white p-6 shadow-xs transition hover:border-blue-400 hover:shadow-md lg:flex-row lg:items-center lg:justify-between">
                <div className="max-w-2xl">
                  <h3 className="font-serif text-xl font-bold text-[#082352] sm:text-2xl">
                    Chương trình Học bổng & Đề tài “Khai sáng” (Khaisang Initiative)
                  </h3>
                  <p className="mt-2 text-sm leading-relaxed text-slate-600 sm:text-base">
                    Sáng kiến của Quỹ Thúc đẩy Hợp tác Nga - Việt tài trợ kinh phí sinh hoạt, hỗ trợ bảo vệ luận án và kết nối phòng thí nghiệm cho các nghiên cứu sinh, nhà khoa học trẻ tại LB Nga.
                  </p>
                </div>
                <div className="flex shrink-0 flex-wrap sm:flex-nowrap items-center gap-3 lg:justify-end">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setConnectForm((prev) => ({
                        ...prev,
                        message: "Tôi muốn tìm hiểu thông tin và nộp hồ sơ xin xét duyệt Chương trình Học bổng Khai sáng.",
                      }));
                    }}
              ...
              <article className="flex flex-col gap-5 rounded-2xl border border-blue-200/90 bg-white p-6 shadow-xs transition hover:border-blue-400 hover:shadow-md lg:flex-row lg:items-center lg:justify-between">
                <div className="max-w-2xl">
                  <h3 className="font-serif text-xl font-bold text-[#082352] sm:text-2xl">
                    Dự án Hợp tác Nghiên cứu Năng lượng Sạch & Pin Thế hệ Mới Nga – Việt
                  </h3>
                  <p className="mt-2 text-sm leading-relaxed text-slate-600 sm:text-base">
                    Chương trình tài trợ liên kết giữa VAST và MISIS / SPbPU nhằm chế tạo vật liệu lưu trữ năng lượng bền vững và pin trạng thái rắn (kinh phí dự kiến lên tới 15.000.000 RUB).
                  </p>
                </div>
                <div className="flex shrink-0 flex-wrap sm:flex-nowrap items-center gap-3 lg:justify-end">
                  <Link
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setConnectForm((prev) => ({
                        ...prev,
                        field: "Vật liệu tiên tiến & Năng lượng",
                        message: "Tôi quan tâm đến Gói tài trợ Đề tài Năng lượng Sạch & Pin thế hệ mới và muốn đăng ký tham gia.",
                      }));
                    }}
                    className="inline-flex items-center justify-center rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white shadow-xs transition hover:bg-blue-700"
                  >
              ...
              <article className="flex flex-col gap-5 rounded-2xl border border-blue-200/90 bg-white p-6 shadow-xs transition hover:border-blue-400 hover:shadow-md lg:flex-row lg:items-center lg:justify-between">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-slate-600 sm:text-base">
                    Sáng kiến thành lập mạng lưới phòng thí nghiệm ảo về AI, dịch máy đa ngữ Nga – Việt và phân tích dữ liệu quan trắc viễn thám không gian giữa Đại học Quốc gia Hà Nội và ĐH Bauman.
                  </p>
                </div>
                <div className="flex shrink-0 flex-wrap sm:flex-nowrap items-center gap-3 lg:justify-end">
                  <Link
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setConnectForm((prev) => ({
                        ...
                        message: "Tôi muốn tham gia Nhóm nghiên cứu thuộc Dự án Trung tâm Đổi mới AI & Xử lý Dữ liệu lớn.",
                      }));
                    }}
                    className="inline-flex items-center justify-center rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white shadow-xs transition hover:bg-blue-700"
                  >
        ...
        >
          <div className="mx-auto max-w-[1460px]">
            <div className="text-center mb-10">
              ...
              <p className="mt-3 text-base text-slate-600 sm:text-lg max-w-2xl mx-auto">
                Cơ sở dữ liệu các công trình nghiên cứu, chuyên san học thuật và danh mục sở hữu trí tuệ, sáng chế được công nhận giữa Việt Nam và Liên bang Nga.
              </p>
            </div>
            ...
            {libSubTab === "articles" && (
              <div className="grid gap-6 md:grid-cols-3">
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-lg font-bold leading-snug text-[#082352]">
                      Functional composites and surface engineering for extreme thermal environments
                    </h3>
                    <div className="mt-2 text-xs font-medium text-slate-500">
                      Tác giả: GS.TS Nguyễn Văn An, Prof. Elena Kurchatova
                    </div>
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Nghiên cứu cấu trúc composite nền gốm chịu nhiệt độ cực cao ứng dụng trong động cơ đẩy và vỏ bọc bảo vệ nhiệt thế hệ mới.
                    </p>
                  </div>
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-lg font-bold leading-snug text-[#082352]">
                      Multisource remote sensing and machine learning for marine environmental observation
                    </h3>
                    <div className="mt-2 text-xs font-medium text-slate-500">
                      Tác giả: PGS.TS Bùi Bảo Thiện, PGS.TS Lê Thị Mai
                    </div>
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Tích hợp dữ liệu viễn thám radar và quang học kết hợp mạng nơ-ron học sâu để dự báo xâm nhập mặn và biến động rạn san hô ven bờ.
                    </p>
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-3">
                    <Link
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-lg font-bold leading-snug text-[#082352]">
                      Quantum computing algorithms and simulation frameworks for bilateral research
                    </h3>
                    <div className="mt-2 text-xs font-medium text-slate-500">
                      Tác giả: Denis Avetisyan, PGS.TS Ngạc An Bang
                    </div>
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Khung mô phỏng tính toán lượng tử hỗ trợ phát triển thuốc sinh học và giải mã các cấu trúc phân tử phức tạp.
                    </p>
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-3">
                    <Link
              ...
              <div className="grid gap-6 md:grid-cols-3">
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    ...
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Ấn phẩm học thuật quốc tế xuất bản 4 số/năm bằng tiếng Anh và tiếng Nga, giới thiệu các kết quả nghiên cứu hợp tác mới nhất giữa hai nước.
                    </p>
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-3">
                    <span className="text-xs font-extrabold text-slate-500">ISSN: 2831-9042 · Chỉ mục Scopus</span>
                  </div>
                </article>
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    ...
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Tổng hợp toàn văn các báo cáo khoa học của các nhà khoa học hai nước trong các kỳ hội nghị song phương thường niên.
                    </p>
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-3">
                    <span className="text-xs font-extrabold text-slate-500">ISBN: 978-604-913-882-1</span>
                  </div>
                </article>
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    ...
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Bản tin phân tích xu hướng công nghệ cao của Nga, chính sách khoa học và cơ hội chuyển giao công nghệ cho doanh nghiệp Việt Nam.
                    </p>
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-3">
                    <span className="text-xs font-extrabold text-slate-500">Phát hành hàng tháng</span>
                  </div>
                </article>
              </div>
            )}
            ...
            {libSubTab === "patents" && (
              <div className="grid gap-6 md:grid-cols-3">
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Quy trình tổng hợp vật liệu composite gốm nền cacbua siêu chịu nhiệt
                    </h3>
                    <div className="mt-2 text-xs font-extrabold text-blue-700">
                      Mã bằng: RU2789124 / VN-45892
                    </div>
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Chủ sở hữu: Viện Hàn lâm KHCN Việt Nam & Đại học Bauman. Bảo hộ độc quyền quy trình thiêu kết áp suất cao cho linh kiện hàng không.
                    </p>
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-3">
                    <span className="text-xs font-bold text-emerald-700">Trạng thái: Đang bảo hộ hiệu lực</span>
                  </div>
                </article>
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Thiết bị bay không người lái tự hành thu thập mẫu nước biển tầng nông
                    </h3>
                    <div className="mt-2 text-xs font-extrabold text-blue-700">
                      Mã đăng ký: VN-GPHI-2026-089
                    </div>
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Tác giả: Nhóm nghiên cứu NCS Phạm Quốc Phòng & Viện Hải dương học. Thiết kế cơ cấu thả mẫu tự cân bằng trong điều kiện sóng cấp 4.
                    </p>
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-3">
                    <span className="text-xs font-bold text-blue-700">Trạng thái: Đã cấp bằng</span>
                  </div>
                </article>
                ...
                <article className="flex flex-col justify-between rounded-3xl border border-blue-200 bg-white p-7 shadow-xs transition hover:-translate-y-1 hover:border-blue-400 hover:shadow-md">
                  <div>
                    <h3 className="font-serif text-xl font-bold leading-snug text-[#082352]">
                      Thuật toán giải mã tín hiệu quang học cho cảm biến lượng tử độ nhạy cao
                    </h3>
                    <div className="mt-2 text-xs font-extrabold text-blue-700">
                      Mã bằng: RU2801452-C1
                    </div>
                    <p className="mt-3 text-xs leading-relaxed text-slate-600 sm:text-sm">
                      Chủ sở hữu: Rosatom Quantum JV & Trường ĐH Khoa học Tự nhiên ĐHQGHN. Ứng dụng trong thiết bị đo địa từ trường và địa chất ngầm.
                    </p>
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-3">
                    <span className="text-xs font-bold text-purple-700">Trạng thái: Đang bảo hộ hiệu lực</span>
                  </div>
                </article>
              </div>
            )}
          ...
          <div className="relative flex max-h-[90vh] w-full max-w-4xl flex-col rounded-3xl border border-blue-200 bg-white p-6 shadow-2xl sm:p-8">
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-5">
              <div>
                ...
                <h3 id="results-modal-title" className="mt-1 font-serif text-2xl font-black text-[#082352] sm:text-3xl">
