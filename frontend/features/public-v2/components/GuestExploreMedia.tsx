        ...
        <div className="mx-auto max-w-[1460px]">
          <RevealItem delay={50}>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div className="max-w-3xl">
                ...
                <p className="mt-3 text-lg sm:text-xl md:text-[20px] font-normal leading-relaxed text-slate-700">Không gian hình ảnh dành cho hoạt động nghiên cứu, hội thảo, trao đổi chuyên gia và những điểm chạm nổi bật của mạng lưới.</p>
              </div>
              <Link href="/experts" className="inline-flex min-h-12 shrink-0 items-center justify-center self-start rounded-xl border border-blue-300/90 bg-white/95 px-6 text-sm sm:text-base font-bold text-blue-700 shadow-2xs transition hover:border-blue-400 hover:bg-white hover:shadow-xs sm:self-auto">
                Khám phá mạng lưới →
              </Link>
            </div>
          </RevealItem>
          ...
          <div className="mt-8 grid auto-rows-[220px] gap-5 sm:grid-cols-2 lg:grid-cols-4 lg:auto-rows-[250px]">
            {moments.map((item, index) => (
              ...
              >
                <article
                  className="group relative isolate h-full min-h-[220px] overflow-hidden rounded-[24px] border border-blue-100 bg-blue-100 shadow-[0_22px_55px_-40px_rgba(37,99,235,.85)]"
                >
                  <div
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-5 sm:p-6">
                    <span className={`inline-flex rounded-full border border-white/25 ${item.badgeColor} px-3.5 py-1 text-xs sm:text-[13px] font-black uppercase tracking-wider backdrop-blur`}>{item.tag}</span>
                    <h3 className="mt-2.5 text-lg sm:text-xl md:text-2xl xl:text-[26px] font-bold leading-snug text-white">{item.title}</h3>
                    <p className="mt-2 text-base sm:text-lg font-medium text-blue-100">{item.meta}</p>
                  </div>
                </article>
              </RevealItem>
            ))}
                  ...
                  <div className="mt-5 flex max-w-3xl gap-2.5 overflow-hidden">
                    <div className="guest-banner-slide flex w-max gap-2.5">
                      {[...bannerItems, ...bannerItems].map((item, index) => (
                        <span key={`${item}-banner-${index}`} className="shrink-0 rounded-full border border-white/25 bg-white/15 px-4 py-2 text-sm sm:text-base font-bold text-white backdrop-blur">{item}</span>
                      ))}
                    </div>
                  </div>
                </div>
