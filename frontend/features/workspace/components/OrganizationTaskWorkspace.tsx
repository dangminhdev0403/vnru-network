...
function OrganizationTaskWorkspaceContent() {
  ...
  const [selectedProjectCode, setSelectedProjectCode] = React.useState<string | null>(null);
  ...
  return (
    <main className="w-full space-y-7 px-5 py-7 md:px-8 lg:px-10 xl:px-12">
      ...
      {selectedProject && <WorkspaceTaskDialog title={selectedProject.title} eyebrow={`Dự án tổ chức · ${selectedProject.code}`} tone="teal" onClose={() => setSelectedProjectCode(null)} footer={<button type="button" onClick={() => setSelectedProjectCode(null)} className="min-h-10 rounded-lg bg-teal-700 px-4 text-sm font-bold text-white hover:bg-teal-800">Đóng</button>}><div className="space-y-5"><div className="grid gap-3 sm:grid-cols-3"><Metric value={`${selectedProject.progress}%`} label="Tiến độ" detail="Dữ liệu minh họa" /><Metric value={selectedProject.partner} label="Đối tác" detail="Đơn vị phía Nga" /><Metric value={selectedProject.state} label="Trạng thái" detail="Theo dõi trong UI Preview" /></div><div className="rounded-xl border border-teal-200 bg-teal-50 p-4 dark:border-teal-900 dark:bg-teal-950/30"><p className="text-xs font-extrabold uppercase tracking-wider text-teal-700 dark:text-teal-300">Việc tiếp theo của tổ chức</p><p className="mt-2 text-sm font-bold leading-6 text-teal-950 dark:text-teal-100">{selectedProject.next}</p></div><div className="rounded-xl border border-card-border p-4"><h3 className="text-sm font-bold text-slate-900 dark:text-white">Tài liệu theo dõi mẫu</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600 dark:text-slate-300"><li>• Biên bản phối hợp tổ chức VN–RU</li><li>• Tóm tắt mốc tiến độ gần nhất</li><li>• Danh sách nguồn lực cần xác nhận</li></ul></div></div></WorkspaceTaskDialog>}
