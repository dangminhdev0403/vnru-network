          ...
          >
            {navItems.map((item) => (
              <Link
                ...
                className={`transition-colors hover:text-white ${item.active ? "text-white underline decoration-sky-400 decoration-2 underline-offset-8" : ""}`}
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="mx-auto flex max-w-[1380px] gap-6 overflow-x-auto px-4 py-3 text-xs font-bold text-slate-200 sm:px-6">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                aria-current={item.active ? "page" : undefined}
                className={`shrink-0 transition-colors hover:text-white ${item.active ? "text-white underline decoration-sky-400 decoration-2 underline-offset-4" : ""}`}
              >
                {item.label}
              </Link>
            ))}
