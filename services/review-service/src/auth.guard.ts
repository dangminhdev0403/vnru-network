...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const contextType = this.reflector.getAllAndOverride<string>('contextType', [context.getHandler(), context.getClass()]);
    const capability = this.reflector.getAllAndOverride<string>('capability', [context.getHandler(), context.getClass()]);
    if (contextType && user.activeContext?.contextType !== contextType) throw new ForbiddenException(`Active context must be ${contextType}`);
