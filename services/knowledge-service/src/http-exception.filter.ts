...
export class SafeHttpExceptionFilter implements ExceptionFilter {
    ...
    return response.status(unavailable ? 503 : 500).json({ error: { code: unavailable ? 'SERVICE_UNAVAILABLE' : 'INTERNAL_ERROR', message: unavailable ? 'Service unavailable' : 'Internal service error' } });
  }
}
